...
BasicFS::BasicFS(const string& filename,
             const uint fs_size,
...
void BasicFS::init_disk(const string& filename) {
...
unique_ptr<BasicFS::PathRet> BasicFS::parse_path(string path_str) const {
...
bool BasicFS::getMode(Mode *mode, string mode_s) {
  if (mode_s == "w") {
    *mode = W;
  } else if(mode_s == "r") {
    *mode = R;
...
bool BasicFS::basic_open(Descriptor *d, vector <string> args) {
  assert(args.size() == 3);
...
void BasicFS::open(vector<string> args) {
  ops_exactly(2);
  Descriptor desc;
...
void BasicFS::read(vector<string> args) {
  ops_exactly(2);
...
unique_ptr<string> BasicFS::basic_read(Descriptor &desc, const uint size) {
...
void BasicFS::write(vector<string> args) {
  ops_exactly(2);
...
uint BasicFS::basic_write(Descriptor &desc, const string data) {
...
void BasicFS::seek(vector<string> args) {
  ops_exactly(2);
  uint fd;
  if ( !(istringstream(args[1]) >> fd)) {
...
bool BasicFS::basic_close(uint fd) {
...
void BasicFS::close(vector<string> args) {
  ops_exactly(1);
  uint fd;
...
void BasicFS::mkdir(vector<string> args) {
  ops_at_least(1);
...
void BasicFS::rmdir(vector<string> args) {
  ops_at_least(1);
...
void BasicFS::printwd(vector<string> args) {
  ops_exactly(0);
...
void BasicFS::cd(vector<string> args) {
  ops_exactly(1);
...
void BasicFS::link(vector<string> args) {
  ops_exactly(2);
...
void BasicFS::unlink(vector<string> args) {
  ops_exactly(1);
...
void BasicFS::stat(vector<string> args) {
  ops_at_least(1);
...
void BasicFS::ls(vector<string> args) {
  ops_exactly(0);
...
void BasicFS::cat(vector<string> args) {
  ops_at_least(1);
...
void BasicFS::cp(vector<string> args) {
  ops_exactly(2);
...
void BasicFS::tree(vector<string> args) {
  ops_exactly(0);

  tree_helper(pwd, "");
}

void BasicFS::import(vector<string> args) {
  ops_exactly(2);
...
void BasicFS::FS_export(vector<string> args) {
  ops_exactly(2);
