...
int test_fs(const string filename) {
  BasicFS myfs(filename, DISKSIZE, BLOCKSIZE, DIRECTBLOCKS);
...
void repl(const string filename) {
  ...
  BasicFS *fs = new BasicFS(filename, DISKSIZE, BLOCKSIZE, DIRECTBLOCKS);
    ...
    while (getline(cin, cmd)) {
        ...
        if (args[0] == "mkfs") {
            if (args.size() == 1) {
                delete(fs);
                fs = new BasicFS(filename, DISKSIZE, BLOCKSIZE, DIRECTBLOCKS);
            } else {
